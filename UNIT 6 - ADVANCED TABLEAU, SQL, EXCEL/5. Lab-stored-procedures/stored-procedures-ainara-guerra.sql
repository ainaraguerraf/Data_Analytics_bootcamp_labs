# STORED PROCEDURE | AINARA GUERRA
...
CREATE PROCEDURE GetCustomersByAction()
BEGIN
...
# I took some help from internet + chatGPT
...
CREATE PROCEDURE Countingmovies(IN minMovieCount INT)
BEGIN
    SELECT category.name, COUNT(*) AS movie_count
    FROM category
    JOIN film_category ON category.category_id = film_category.category_id
    GROUP BY category.name
    HAVING COUNT(*) > minMovieCount;
END //

DELIMITER ;


CALL Countingmovies(10);
